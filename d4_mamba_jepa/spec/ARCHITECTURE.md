...
diff   `imagine_trajectory` re-slices `[:, -context:]` each step and `_sample_next_jepa` carries no recurrent cache, so the rollout is a sliding 8-state window re-scanned from a zero SSM state. Generated latents stay inside that window, so the M arm is not memoryless — what it never accumulates is recurrent SSM state, or any information older than 8 steps, across the 32-step horizon. `sample_next_packed` is still called with `use_cache=True`; the JEPA branch ignores it
...
diff   only `checkpoint.py`'s world/tokenizer writers carry full RNG. The BC, actor and value heads store format + config + paired world digest, no RNG and no source report
