...
| S67 | Shortcut training partitions **rows**, not positions, and bootstraps only after `bootstrap_start` | The pinned mmbench2 source is explicit -- "empirical rows are finest (d_min), self rows sample coarser" -- with `self_fraction=0.25` and `bootstrap_start=10_000`: three quarters of *rows* are supervised against the clean latent at the finest step, one quarter bootstrap at coarser steps, and none bootstrap during warmup. We sampled the step size independently per position, so ~75% of positions took a bootstrap target from the first update, chasing an untrained model's own predictions. The consequence is measurable: on both flow arms after Phase 1B, *more* denoising made rollouts worse -- flow-attention 0.522 / 0.576 / 0.648 / 0.691 at 1/2/4/8 rungs, flow-mamba 0.551 / 0.630 / 0.683 / 0.699 -- which is what an incoherent shortcut ladder looks like. Phase 2 must continue the same clock, or it restarts the warmup and drops back to the supervised objective |
...
| S70 | **Phase-2 outcome supervision is the first universal collapse in the 20k DEV run** | An environment fork executed all 17 actions at 36 identical real states. Encoded action effects remain nonzero and matched-action successor MSE beats off-action MSE in every arm, but true death risk under the actors is 13.1–14.4% while the continuation heads predict only 0.046–0.070% death even on the true encoded successors. Counterfactual reward correlation is also weak. Logged-action calibration therefore does not gate the distribution Phase 3 optimises. Raw measurements are preserved in `artifacts/stage_a_olddesign/counterfactual_forks_preterminalfix.json`; the next run must strengthen terminal supervision and refuse Phase 3 unless a real simulator-fork outcome gate passes |
