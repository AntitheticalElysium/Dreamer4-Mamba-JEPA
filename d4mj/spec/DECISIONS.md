...
| **Uniform-half collection settings** — the *policy* is settled (S29), the *target* (S46) and the loader (S49) are settled, and S50 shows terminal exposure is the binding shortage. Still open: which policy generates the uniform half (random, early PPO checkpoints, epsilon-mixed expert, or a blend), the episode counts per half, the acceptance threshold defining "relevant" on Craftax's 22 achievements, and the provenance record | Phase 1A | `expert.collect` |
