...
| S72 | **Terminal supervision is a stratified Bernoulli likelihood, not a positive-class weight** | D4 names nonterminal `c_t` but does not specify its estimator. The closest pinned implementation is Dreamer 3's ordinary binary continuation likelihood. We retain it on the main mixture and add one tail-aligned terminal sequence per Phase-2 update at the same committed condition deployment reads. BCE covers the tail's terminal **and nonterminal** targets; with four main rows plus one tail, equal per-sequence weighting gives the stratum 20% of continuation loss and about 1.0% actual terminal-target mass under the short/long schedule, versus 61.5% in the predecessor's failed ablation. It never displaces BC or dynamics rows and cannot enter their losses. Tail stratification is a declared Craftax adaptation, not attributed to D4 |
