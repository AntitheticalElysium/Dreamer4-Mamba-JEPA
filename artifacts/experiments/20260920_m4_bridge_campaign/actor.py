...
trained. Direct checks `horizon <= direct_rollout`; here the equivalent bound is the recursive
schedule's final depth, and it is checked rather than assumed.
"""
...
from d4mj.data import atomic_manifest, load_joint_corpus, sample_batch
...
def load_bridge(path: Path, device: str):
    """A bridge checkpoint: the continued world and the fitted heads."""
...
def main(argv=None):
    ...
    if settings.horizon > payload.get("depth", settings.recursive_depth_final):
        raise SystemExit(f"horizon {settings.horizon} exceeds the {payload.get('depth')} generated "
                         "states this bridge trained; raise the recursive schedule first")
    # Streaming inference uses running statistics; `advance` requires it and deployment does too.
    bundle.world.eval()
    ...
    episodes, _ = load_joint_corpus(args.corpus, recipe)
    ...
    atomic_manifest(args.out / "actor.json", {
        "schema": "d4mj_m4_actor_v1", "bridge": str(args.bridge), "variant": recipe.variant,
        "steps": steps, "horizon": settings.horizon,
        "trained_generated_depth": payload.get("depth"),
        "frozen": ["encoder", "world", "reward/continuation body", "prior policy"],
    ...
    for step in range(steps):
        ...
        objective.backward()
        norm = torch.nn.utils.clip_grad_norm_(heads.actor_parameters(), 1.0)
        optimizer.step()

        row = {"step": step + 1, **{k: float(v.detach()) for k, v in losses.items()},
        ...
        if (step + 1) % args.checkpoint_every == 0 or step + 1 == steps:
            torch.save({"heads": heads.state_dict(), "prior": prior.state_dict(),
                        "world": bundle.world.state_dict(), "step": step + 1,
                        "variant": recipe.variant, "config": payload["config"],
                        "bridge": str(args.bridge)},
                       args.out / f"actor_{step + 1:06d}.pt")
    log.close()
