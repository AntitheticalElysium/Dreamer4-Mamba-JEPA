...
| Reward/continuation heads on 2-register pool | Structural precedent: DreamerV3/CDP and DRAMA; exact reward distribution: DRAMA + unofficial Dreamer-4 JAX | Consume pooled POST-TEMPORAL context; weights 1.0 in every validated dynamics run and backpropagate into the temporal core. Exact reward operator is uniform symlog bins + symlog target + `symexp(E[symlog])`. DreamerV3/CDP instead interpolates in original reward space over `symexp`-spaced bins and decodes `E[reward]`; they are not equivalent for uncertain distributions. See `2026-07-18-reward-head-source-correction.md` | Gradient path verified (temporal-core grad sum 8.41 under reward-only backward); Stage-2C/2D show discrimination/calibration and shared-representation trade-offs | "Reward/continuation already train the temporal core; local reward categorical is DRAMA/unofficial-D4-JAX-aligned, not exact DreamerV3 or official Dreamer 4." Predictive calibration remains unresolved |
...
## 2026-07-19 Stage-2E independent ruling

- ISOLATION: PASS after repairing CPU/CUDA decode parity. E-I reproduces the
  committed C-LR natural reward predictions and fork rows exactly; the full
  world-state digest is unchanged.
- CAL SELECTION: deterministic E-TZ, `T=1.4998668`, zero-bin bias `1.2552725`,
  NLL `.187375935` versus identity `.195777025`.
- DEPLOYMENT: FAIL. Absolute predicted return on zero-return suffixes improves
  `.06404->.04177`, but remains `+.03230 [.02432,.04041]` above A and exceeds
  the `+.02` budget. K8 Pearson improves, while event MAE significantly
  worsens and decoded event magnitude falls.
- INTERPRETATION: two global scalars cannot separate C-LR's useful reward
  ordering from its unsafe false-return/magnitude trade-off. Do not select a
  transparent non-winning arm or sweep thresholds on spent DEV.
- The next source-backed control, if pursued, must jointly retrain the local
...
## 2026-07-20 D4-lite positive-control ruling

- BASELINE: GO on the small source-pinned CartPole track. The frozen
  Transformer representation plus categorical BC head achieves 288.70 mean
  return versus 17.60 random on 30 fresh seeds, wins all pairs, and reproduces
  exactly. This is now the required positive control for architecture swaps.
- IMAGINATION: POSITIVE BUT BELOW ITS ABSOLUTE GATE. Exhaustive horizon-8
  planning achieves 44.87 versus 19.00 random, paired +25.87
  [15.80,36.73], but misses mean return 50. Do not relabel the policy result as
  imagination-trained control.
- SOURCE CORRECTION: task heads now consume agent tokens from the same noised
  shortcut-flow forward as MMBench2. Older Crafter checkpoints remain bound to
  their historical implementation hashes.
- CLAIM BOUNDARY: this licenses a small D4-style learned pixel-control
  baseline, not a full Dreamer 4 reproduction, solved raw-RGB transfer, or
  online actor/value learning.
- NEXT MODIFICATION RULE: Mamba and JEPA/CDP remain off in the control
